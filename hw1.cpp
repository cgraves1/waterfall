...
int main(void)
{
	...
	defineBox(&game.box[1], 100, 20, -100+5*65, 720-5*65);
...
void defineBox(Shape *tmpBox, float w, float h, float cenX, float cenY)
{
	tmpBox->width = w;
	tmpBox->height = h;
	tmpBox->center.x=cenX;
	tmpBox->center.y=cenY;
}

void defineCircle(Shape *tmpCircle, float rad, float cenX, float cenY)
{
	tmpCircle->radius = rad;
	tmpCircle->center.x = cenX;
	tmpCircle->center.y = cenY;
}
...
void makeParticle(Game *game, int x, int y)
{
	...
	p->s.center.x = x;
	p->s.center.y = y;
	p->velocity.y = rnd() * 1.0 - 0.5; //random value -1, 0, or 1
